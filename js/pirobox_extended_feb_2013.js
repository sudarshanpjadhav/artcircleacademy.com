...
(function($) {
	...
	var map = new Object();
		for (var i=0; i<my_gall_obj.length; i++) {
			...
			map['a.'+ it.attr('class').match(/^pirobox_gall1\w*/)]=0;
		}
	...
	$('a[class*="pirobox_gall1"]').each(function(rev){
			
		this.rev = rev+0});
	if($('a[class*="pirobox_gall1"]').length == 1){ 
	$('a[class*="pirobox_gall1"]').each(function(){
	...
	$(piro_prev).add(piro_next).bind('click',function(c) {
		...
		var obj_count = parseInt($('a[class*="pirobox_gall1"]').filter('.item').attr('rev'));
		var start = $(this).is('.piro_prev') ? $('a[class*="pirobox_gall1"]').eq(obj_count - 1) : $('a[class*="pirobox_gall1"]').eq(obj_count + 1);
		start.click();
	});
